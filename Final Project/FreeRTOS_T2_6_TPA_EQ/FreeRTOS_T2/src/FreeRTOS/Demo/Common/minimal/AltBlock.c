...
portTickType xTimeToBlock, xBlockedTime;
	...
	for( ;; )
	{
		...
		for( xItem = 0; xItem < bktQUEUE_LENGTH; xItem++ )
		{
			if( xQueueSendToBack( xTestQueue, &xItem, bktDONT_BLOCK ) != pdPASS )
			{
				xErrorOccurred = pdTRUE;
			}
		}
		...
		for( xItem = 0; xItem < bktQUEUE_LENGTH; xItem++ )
		{
			...
			portENTER_CRITICAL();
			{
				...
				/* We should unblock after xTimeToBlock having not received
				anything on the queue. */
				if( xQueueSendToBack( xTestQueue, &xItem, xTimeToBlock ) != errQUEUE_FULL )
				{
					xErrorOccurred = pdTRUE;
				}
...
portBASE_TYPE xData;
	...
	for( ;; )
	{
		...
		portENTER_CRITICAL();
		{
			...
			if( xQueueSendToBack( xTestQueue, &xData, bktTIME_TO_BLOCK ) != errQUEUE_FULL )
			{
				xErrorOccurred = pdTRUE;
			}
